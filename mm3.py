...
def hash( key : bytearray, seed = 0x0 ) -> int:
    ''' 32bit hash '''
...
def hash128( key: bytearray, seed = 0x0, x64arch : bool = True ) -> int:
    ''' 128bit hash. '''
    def hash128_x64( key: bytearray, seed ) -> int:
        ''' 128bit hash @ x64 '''
    ...
    def hash128_x32( key: bytearray, seed ) -> int:
        ''' 128bit hash @ x32 '''
    ...
    if x64arch:
        ...
        return hash128_x32( key, seed )


def hash64( key : bytearray, seed = 0x0, x64arch : bool = True ) -> tuple:
    ''' 64bit hash '''
...
def hash_bytes( key : bytearray, seed = 0x0, x64arch : bool = True ) -> str:
    ''' 128bit hash'''
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser( 'mm3', 'mm3 [options] "string -> hash"' )
    ...
    for str_to_hash in opts.strings:
        sys.stdout.write( '"%s" =  %s\n' % ( str_to_hash, xencode(hash_bytes(str_to_hash))))